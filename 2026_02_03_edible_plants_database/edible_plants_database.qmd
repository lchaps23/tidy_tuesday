...
```{r}
sunlight_temp_avgs <- function(df){
  ...
  df |> 
    ...
    summarise(
      n_plants = n(),
      mean_grow_temp = round(mean(grow_temp, na.rm = TRUE), 2),
      sd_grow_temp   = round(sd(grow_temp, na.rm = TRUE), 2),
      mean_germ_temp = round(mean(germ_temp, na.rm = TRUE), 2),
      sd_germ_temp   = round(sd(germ_temp, na.rm = TRUE), 2)
    ) |> 
...
```{r}
sunlight_temp_avgs(edible_plants)
```


```{r}
res.aov <- aov(grow_temp ~ sunlight, data = edible_plants)
