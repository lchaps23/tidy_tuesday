...
## Top 15
```{r}
#| label: top_15_countries

top_15_countries <- africa |> 
...
## Diving further into Cameroon's languages

```{r}
#| label: cameroon_languages

cameroon_languages <- africa |> 
  
  filter(country == "Cameroon") |> 
  
  distinct(language, family, country) |> 
  
  count(family, name = "family_n") |> 
  
  mutate(prop = round(family_n / sum(family_n), 2)) |> 
...
cameroon_languages |> 
  ...
  formatPercentage("% of Languages", 0)
```

# Which family of languages has the highest density of speakers?

```{r}
#| label: speaker_density

speaker_density <- africa |> 
  
  distinct(native_speakers, family) |> 
  
  group_by(family) |> 
  
  summarise(speaker_density = round((sum(native_speakers) / 1000000), 1)) |> 
  
  arrange(desc(speaker_density)) |> 
  
  slice_max(speaker_density, n = 10)

speaker_density |> 
  
  rename(
    "Language Family" = "family",
    "Number of Speakers (Millions)" = "speaker_density"
  ) |> 
